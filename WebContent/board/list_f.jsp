...
List boardList=fbdao.getBoardList(startRow, pageSize);
%>
<table border="1">
...
// 세션값이 있으면 글쓰기 버튼 보이기
if(id!=null){
	%><input type="button" value="파일업로드" class="btn" 
...
if(startPage > pageBlock){
	%> <a href="list_f.jsp?pageNum=<%=startPage-pageBlock%>">[이전]</a> <%
}

// 1~ 10  11~20   startPage  ~ endPage
for(int i=startPage;i<=endPage;i++){
	%> <a href="list_f.jsp?pageNum=<%=i%>"><%=i%></a> <%
}

//[다음] 10페이지 다음
if(endPage < pageCount){
	%> <a href="list_f.jsp?pageNum=<%=startPage+pageBlock%>">[다음]</a> <%
}
%>
</body>
</html>
